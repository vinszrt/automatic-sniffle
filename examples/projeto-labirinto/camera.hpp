...
class Camera {
 ...
 private:
  ...
  glm::vec3 m_eye{glm::vec3(0.0f, 1.6f, 2.0f)};  // Camera position
  glm::vec3 m_at{glm::vec3(0.0f, 1.6f, 0.0f)};   // Look-at point
...
void Camera::dolly(float speed, std::array<glm::vec3, SIZE> m_wallPositions) {
  ...
  const glm::vec2 proj_wall = {m_wallPositions.at(0)[0],
                               m_wallPositions.at(0)[2]};

  printf("%s",
         fmt::format("Distance == {}\n", glm::distance(proj_eye, proj_wall))
             .c_str());
...
void Camera::truck(float speed, std::array<glm::vec3, SIZE> m_wallPositions) {
  ...
  if (glm::distance(m_next_eye, m_wallPositions.at(0)) > 0.71) {
